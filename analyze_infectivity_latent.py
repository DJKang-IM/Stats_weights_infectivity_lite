...
def _parse_prior_json(s: str) -> dict[str, float]:
    """
    Parse --constrained-prior.
    Accepts:
      - JSON object string: {"Liquid_Culture": 1, ...}
      - PowerShell-escaped JSON: {\"Liquid_Culture\":1,...}
    """
    raw = (s or "").strip()
    if not raw:
        return {}
    try:
        ...
        # common case: quotes were escaped when passed through a shell
        obj = json.loads(raw.replace('\\"', '"'))
    if not isinstance(obj, dict):
        raise ValueError("--constrained-prior must decode to a JSON object mapping.")
    out: dict[str, float] = {}
...
def fit_constrained_cfa_sem(
    Z_df: pd.DataFrame,
    *,
    fixed_loadings: dict[str, float],
) -> tuple[object, pd.DataFrame, pd.DataFrame | None, pd.DataFrame | None]:
    """
    Constrained CFA: fix reflective loadings to user-provided values.

    Important:
    - Z_df should be standardized indicators.
    - fixed_loadings keys must include all COLS.
    - Latent variance is free (scale set by fixed loadings); semopy will estimate residual variances.
    """
    from semopy import Model
    from semopy.stats import calc_stats

    miss = [c for c in COLS if c not in fixed_loadings]
    if miss:
        raise ValueError(f"fixed_loadings missing keys: {miss}")

    # Use Liquid=1 by convention, others relative (e.g., Liquid/X or any clinical prior).
    # Fix all loadings numerically to enforce the prior.
    parts = [f"{float(fixed_loadings[c]):.10g}*{c}" for c in COLS]
    desc = "Infectivity =~ " + " + ".join(parts) + "\n"
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default="",
        help=(
            "Optional fixed-loading prior for constrained CFA. "
            "Provide JSON mapping of {col: loading} for all 5 COLS (e.g. Liquid=1). "
            "Example: '{\"Liquid_Culture\":1,\"Solid_Culture\":1.27,\"TB_PCR\":1.06,\"AFB_Smear\":1.31,\"Cavity\":2.50}'."
        ),
    )
    ...
    sem_ins_con = None
    sem_ins_con_std = None
    sem_stats_con = None
    std_loadings_con: dict[str, float] = {}
    influence_01_con: dict[str, float] = {}
    try:
    ...
    # --- Constrained CFA (fixed loadings prior) ---
    if (args.constrained_prior or "").strip():
        try:
            prior = _parse_prior_json(args.constrained_prior)
            sem_model_con, sem_stats_con, sem_ins_con, sem_ins_con_std = fit_constrained_cfa_sem(
                Z_df, fixed_loadings=prior
            )
            srmr_c = srmr_from_sigma(Z_df, sem_model_con)
            sem_stats_con = sem_stats_con.copy()
            sem_stats_con["SRMR"] = np.nan
            sem_stats_con.loc[sem_stats_con.index[0], "SRMR"] = srmr_c
            if sem_ins_con_std is not None:
                std_loadings_con = extract_cfa_standardized_loadings(sem_ins_con_std)
                influence_01_con = influence_share_0_1(std_loadings_con)
        except Exception as e:
    ...
    report_lines.extend(
        [
            ...
            "",
            "Constrained CFA (fixed-loading prior) results:",
            ("(not requested)" if not (args.constrained_prior or "").strip() else ""),
            ("Fit indices:\n" + (sem_stats_con.to_string() if sem_stats_con is not None else "(not available)"))
            if (args.constrained_prior or "").strip()
            else "",
            "",
            "Constrained CFA standardized coefficients (Est. Std):",
            json.dumps(std_loadings_con, indent=2, ensure_ascii=False)
            if std_loadings_con
            else ("(not available)" if (args.constrained_prior or "").strip() else "(not requested)"),
            "",
            "Constrained CFA relative influence on [0, 1] (|Est. Std| / sum):",
            json.dumps(influence_01_con, indent=2, ensure_ascii=False)
            if influence_01_con
            else ("(not available)" if (args.constrained_prior or "").strip() else "(not requested)"),
            "",
    ...
    summary = {
        ...
        "constrained_cfa_requested": bool((args.constrained_prior or "").strip()),
        "constrained_cfa_prior_fixed_loadings": (_parse_prior_json(args.constrained_prior) if (args.constrained_prior or "").strip() else {}),
        "constrained_cfa_fit": sem_stats_con.to_dict() if sem_stats_con is not None and "CFI" in sem_stats_con.columns else {},
        "constrained_cfa_standardized_loadings": std_loadings_con,
        "constrained_cfa_relative_influence_0_1": influence_01_con,
